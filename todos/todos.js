...
const validateInput = (input, validation) => {
  // Para seleccionar el p
  const helper = input.parentElement.children[2];

  // Aqui se verifican las validaciones para quitar el disabled del boton
  ...
  // Se valida si el input esta vacio
  // Si la validacion es correcta o erronea
  if (input.value === '') {
...
list.addEventListener('click',  async e => {
  if (e.target.closest('.delete-icon')) {
    const id = e.target.closest('.list-item').id; // Busca el elemento '.list-item' más cercano que contenga el ícono de eliminación
  console.log(id);
    await fetch(`http://localhost:3000/contactos/${id}`, {
        method: "DELETE", 
    });
    e.target.closest('.list-item').remove(); // Borra el elemento más cercano de clase '.list-item'
    localStorage.setItem('contacts', list.innerHTML);
 ...
 if (e.target.closest('.edit-icon')) {
   ...
   if (!editBtn.classList.contains('editando')) {
    ...
    //Asegura valor
     ...
     // Obtiene el ID del contacto que se está editando
    const contactId = editBtn.parentElement.id;

    // Obtiene los nuevos valores del nombre y el número de teléfono
    const updatedName = editName.value;
    const updatedPhone = editPhone.value;

    // Realizar la solicitud de actualización al servidor
    await fetch(`http://localhost:3000/contactos/${contactId}`, {
      method: "PUT", 
      headers: {
        "Content-type": "application/json"
      },
